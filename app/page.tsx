...
const BUSINESS_CATEGORIES = [
  'Retail Store', 'Real Estate', 'Education / Coaching', 'Healthcare / Clinic',
  'Restaurant / Hospitality', 'Manufacturing / Industry', 'E-commerce',
  'Professional Services', 'Other'
];

const BUSINESS_GOALS = [
  'Increase sales', 'Generate more leads', 'Increase brand awareness',
  'Build strong online presence', 'Launch new product/service', 'Expand to new locations'
];

const MARKETING_ACTIVITIES = [
  'Social Media Marketing', 'Google Ads', 'SEO', 'WhatsApp Marketing',
  'Email Marketing', 'Influencer Marketing', 'Offline Marketing only', 'Not doing marketing yet'
];

const DIGITAL_SERVICES = [
  'Social Media Management', 'Website Development',
  'Performance Ads (Google / Meta Ads)', 'SEO (Search Engine Optimization)',
  'WhatsApp Marketing', 'Content Creation (Photos / Videos / Reels)',
...
const initialForm: FormData = {
  businessName: '', businessLocation: '', contactPerson: '', mobileNumber: '', emailAddress: '',
  businessCategory: '', businessCategoryOther: '', businessAge: '', businessGoals: [],
  primaryCustomers: '', targetLocations: [], hasWebsite: '', websiteLink: '',
  usesSocialMedia: '', facebookHandle: '', instagramHandle: '', linkedinHandle: '', youtubeHandle: '',
  marketingActivities: [], marketingManager: '', interestedServices: [],
  monthlyBudget: '', biggestChallenge: '', wantsReview: ''
};
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 to-white p-6">
        <div className="bg-white rounded-3xl shadow-xl p-10 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 mb-6">Your form has been submitted successfully. Our team will review your information and get back to you with a tailored digital marketing strategy.</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-orange-50">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white py-8 px-4 shadow-lg">
        <div className="max-w-2xl mx-auto text-center">
          <div className="text-3xl font-extrabold tracking-tight mb-1">🐾 Boop Org</div>
          <p className="text-orange-100 text-sm">Digital Marketing Discovery Form</p>
          <p className="mt-3 text-orange-50 text-sm max-w-lg mx-auto">
            Please fill out this short form so our team can understand your business and recommend the best digital marketing strategy for you.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        {/* Section 1 */}
        <Section title="Section 1" subtitle="Basic Business Details" color="orange">
          <Field label="Business / Brand Name" required>
            <Input value={form.businessName} onChange={v => set('businessName', v)} placeholder="e.g. Hope Scanning Centre" required />
          </Field>
          <Field label="Business Location (City & State)" required>
            <Textarea value={form.businessLocation} onChange={v => set('businessLocation', v)} placeholder="e.g. Station Road, Gonda, UP - 271001" required />
          </Field>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Contact Person Name" required>
              <Input value={form.contactPerson} onChange={v => set('contactPerson', v)} placeholder="Full name" required />
            </Field>
            <Field label="Mobile Number" required>
              <Input value={form.mobileNumber} onChange={v => set('mobileNumber', v)} placeholder="10-digit number" required type="tel" />
            </Field>
          </div>
          <Field label="Email Address" required>
            <Input value={form.emailAddress} onChange={v => set('emailAddress', v)} placeholder="you@example.com" type="email" required />
          </Field>
        </Section>
        ...
        <Section title="Section 2" subtitle="Business Overview" color="blue">
          ...
          <Field label="What is your primary business goal right now?" hint="Select up to 2">
            <CheckboxGrid
              options={BUSINESS_GOALS}
              values={form.businessGoals}
              onChange={v => {
                if (form.businessGoals.includes(v)) set('businessGoals', toggleArray(form.businessGoals, v));
                else if (form.businessGoals.length < 2) set('businessGoals', toggleArray(form.businessGoals, v));
              }}
            />
          </Field>
        </Section>
        ...
        {/* Section 3 */}
        <Section title="Section 3" subtitle="Your Target Customers" color="purple">
          <Field label="Your primary customers are:" required>
            <RadioGroup
              options={['Individual consumers (B2C)', 'Businesses (B2B)', 'Both']}
          ...
          <Field label="Which locations do you want to target?">
            <CheckboxGrid
              options={['Local city', 'Entire state', 'Pan India', 'International']}
              values={form.targetLocations}
              onChange={v => set('targetLocations', toggleArray(form.targetLocations, v))}
            />
          </Field>
        </Section>

        {/* Section 4 */}
        <Section title="Section 4" subtitle="Current Digital Presence" color="teal">
          ...
          {form.hasWebsite === 'Yes' && (
            <Field label="Please share your website link:">
              <Input value={form.websiteLink} onChange={v => set('websiteLink', v)} placeholder="https://yourwebsite.com" type="url" />
            </Field>
          )}
          <Field label="Do you currently use social media for your business?" required>
            <RadioGroup
          ...
          {form.usesSocialMedia !== 'Not yet' && form.usesSocialMedia !== '' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
        ...
        {/* Section 5 */}
        <Section title="Section 5" subtitle="Current Marketing Efforts" color="rose">
          <Field label="Which marketing activities are you currently doing?">
            <CheckboxGrid
              ...
              onChange={v => set('marketingActivities', toggleArray(form.marketingActivities, v))}
            />
          </Field>
          <Field label="Who currently manages your marketing?" required>
            <RadioGroup
              options={['In-house team', 'Freelancer', 'Marketing agency', 'No one']}
        ...
        {/* Section 6 */}
        <Section title="Section 6" subtitle="Digital Services You Are Interested In" color="indigo">
          <Field label="Which services would you like help with?" hint="Select multiple">
            <CheckboxGrid
              options={DIGITAL_SERVICES}
              values={form.interestedServices}
              onChange={v => set('interestedServices', toggleArray(form.interestedServices, v))}
            />
          </Field>
        </Section>
        ...
        <Section title="Section 7" subtitle="Investment Range" color="amber">
          <Field label="What is your approximate monthly digital marketing budget?" required>
            <RadioGroup
              options={['Less than ₹25,000', '₹25,000 – ₹50,000', '₹50,000 – ₹1,00,000', '₹1,00,000+']}
        ...
        <Section title="Section 8" subtitle="Business Challenges" color="green">
          <Field label="What is your biggest marketing challenge today?" required>
            <RadioGroup
              options={[
        ...
        <Section title="Final" subtitle="Growth Strategy Review" color="orange">
          <Field label="Would you like our team to review your digital presence and suggest a growth strategy?" required>
            <RadioGroup
        ...
        <p className="text-center text-xs text-gray-400 pb-4">
          Boop Org Pvt. Ltd. · Bhutani Alphathum, Sector 90, Noida · <a href="tel:7417437174" className="text-orange-500">7417437174</a>
        </p>
      </form>
    </div>
  );
...
const titleColorMap: Record<string, string> = {
  orange: 'text-orange-600', blue: 'text-blue-600', purple: 'text-purple-600',
  teal: 'text-teal-600', rose: 'text-rose-600', indigo: 'text-indigo-600',
  ...
  return (
    <div className={`rounded-2xl border-l-4 ${colorMap[color]} p-6 shadow-sm`}>
      <div className="mb-5">
        ...
        <h2 className={`text-lg font-bold ${titleColorMap[color]}`}>{subtitle}</h2>
      </div>
...
function RadioGrid({ options, value, onChange, showOtherInput, otherValue, onOtherChange }: {
  options: string[]; value: string; onChange: (v: string) => void;
  showOtherInput?: boolean; otherValue?: string; onOtherChange?: (v: string) => void;
}) {
  return (
    <div>
...
function CheckboxGrid({ options, values, onChange }: { options: string[]; values: string[]; onChange: (v: string) => void }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
      {options.map(opt => {
        ...
        return (
          ...
          >
            <div className={`w-4.5 h-4.5 rounded border-2 flex items-center justify-center flex-shrink-0 ${checked ? 'border-orange-500 bg-orange-500' : 'border-gray-300'}`}>
